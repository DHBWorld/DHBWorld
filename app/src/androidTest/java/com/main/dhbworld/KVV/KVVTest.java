...
public class KVVTest {
   ...
   String data = null;
   boolean loadFromServerFailed = false;

   @Test
   public synchronized void loadFromServerTest() {
      ActivityScenario<KVVActivity> activityScenario = ActivityScenario.launch(KVVActivity.class);

      activityScenario.onActivity(activity -> {
         KVVDataLoader kvvDataLoader = new KVVDataLoader(activity);

         new Thread(() -> {
            try {
               data = kvvDataLoader.loadfromKVVServers(LocalDateTime.now());
            } catch (IOException ignored) {
               loadFromServerFailed = true;
            }
            finished = true;
         }).start();
      ...
      assertNotNull(data);
      assertFalse(loadFromServerFailed);
   }
